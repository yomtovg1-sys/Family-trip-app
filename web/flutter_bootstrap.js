...
_flutter.loader.load({
  ...
  // font" downloads (glyphs no bundled font covers, e.g. plain symbol
  // characters like the ★ rating glyph) to same-origin files instead of
  // Google Fonts — see web/fallback_fonts/. Without this, that one
  // specific mechanism keeps reaching for fonts.gstatic.com even though
  // every font the app actually declares is otherwise bundled locally;
  // offline, those requests fail and the affected glyphs don't render.
  config: {
    fontFallbackBaseUrl: 'fallback_fonts/'
  },
  serviceWorkerSettings: {
    serviceWorkerVersion: "1" /* Flutter's service worker is deprecated and will be removed in a future Flutter release. */
  }
});
